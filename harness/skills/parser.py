...
def parse_allowed_tools(raw: object, skill_file: Path) -> list[str] | None:
    ...
    Returns ``None`` when the field is omitted (legacy allow-all).
    Returns a ``list`` when the field is a YAML sequence of strings,
    including an empty list for explicit no-tool skills.
...
def parse_skill_file(
    ...
    Args:
        ...
        category: ``SkillCategory`` of the skill.
    ...
    try:
        ...
        return Skill(
            ...
            enabled=True,  # Actual state comes from extensions_config.json.
        )

...
from harness.config.extensions_config import ExtensionsConfig, McpOAuthConfig
...
class OAuthTokenManager:
    ...
    async def get_authorization_header(self, server_name: str) -> str | None:
        ...
        async with lock:
            ...
            logger.info(
                "Refreshed OAuth access token for MCP server: %s", server_name
            )
...
async def get_initial_oauth_headers(
    ...
    for server_name in token_manager.oauth_server_names():
        headers[server_name] = (
            await token_manager.get_authorization_header(server_name) or ""
        )

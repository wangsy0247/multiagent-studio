...
# ── Deferred cleanup: keep polling until terminal, then remove ──
...
def _schedule_deferred_cleanup(
    ...
    """Schedule a deferred cleanup that polls until the task is terminal.

    Used when cancellation interrupts a running task — we cannot immediately
    remove it from ``_background_tasks`` because the background thread may
    still be writing to the ``SubAgentResult``.  This function submits a
    one-shot polling job to the scheduler pool.

    Parameters
    ----------
    task_id : str
        The task to eventually clean up.
    timeout_seconds : int
        Execution timeout (used to calculate the max poll count).
    trace_id : str
        Trace ID for logging.
    """
    max_polls = (timeout_seconds + 60) // 5

    def _poll_then_cleanup() -> None:
        for poll_count in range(max_polls):
            result = get_background_task_result(task_id)
            if result is None:
                logger.debug(
                    "[trace=%s] Deferred cleanup: task %s already removed",
                    trace_id, task_id,
                )
                return
            if _is_background_task_terminal(result):
                ...
                logger.debug(
                    "[trace=%s] Deferred cleanup: task %s removed after terminal status",
                    trace_id, task_id,
                )
                return
            import time
        ...
        logger.warning(
            "[trace=%s] Deferred cleanup for task %s timed out after %d polls",
            trace_id, task_id, max_polls,
        )

    _scheduler_pool.submit(_poll_then_cleanup)
    logger.debug(
        "[trace=%s] Scheduled deferred cleanup for task %s",
        trace_id, task_id,
    )
